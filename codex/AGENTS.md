...
- Do not execute the shared skill's Claude Workflow JavaScript or its
  ...
  instructions. Display the source path separately and do not re-read the
  mutable source or duplicate Plan content into every launch prompt.
...
- A reviewer whose TOML uses `sandbox_mode = "workspace-write"` (currently
  `similarity`, `codex-poc`, and `codex-runner`) must run through a native
  Worktree or Handoff whose actual workspace is a dedicated isolated linked
  worktree. Adapter-only creation is insufficient because it does not move the
  child's sandbox root. If this cannot be guaranteed during preflight, start no
  reviewers and report the isolation coverage gap. Retain every plan-review
  worktree, report its absolute path, and never merge or remove it automatically.
- The Claude-only `// codex-skip` marker has no Codex meaning. If the native
  `codex-reviewer` baseline definition is absent, ask the user whether to
  continue with specialist reviewers; without affirmative approval, stop.
