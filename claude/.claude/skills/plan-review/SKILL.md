...
- テストファイルの存在: `*.test.ts`, `*.spec.ts`, `*.test.js`, `*.spec.js`, `*_test.go`, `*_test.rs`
- テスト設定の存在: `vitest.config.ts`, `jest.config.*`, `tests/` ディレクトリ
...
| tdd-reviewer   | テストファイル・テスト設定の存在     | TDD準拠・Testing Trophy・モック最小化・テスト重複検知 |
