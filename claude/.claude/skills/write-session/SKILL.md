...
Mid-session companion to `start-work` and `restoring-session`.
...
Without `write-session`, bit issues stay frozen at session start. Restoring a long session then yields a stale plan and no progress trace, and parallel sessions running overlap detection see stale Target Files.

`write-session` is a save point: it inspects current TaskList, git state, plan file, and the agent's working memory of decisions, then writes the deltas back to the relevant bit issues.
...
Both labelled `session:<branch-name>`. `write-session` reads, never invents new titles or seqs.

## Comment prefix conventions

Use these prefixes so future skills (and `restoring-session`) can parse comment intent. They extend the prefixes already used by `start-work` (`Excluded`, `Target added`) and `restoring-session` (`Re-linked`).
...
Keep each comment ≤ ~5 lines. Snapshots may be slightly longer but never paste raw transcripts or full diffs — use `git diff --stat` summaries and decision bullets only.
...
| No open parent for branch          | Suggest `/start-work` (or restoring-session if user expects one). Stop. |
...
1. **TaskList** — current in-memory tasks (`TaskList` tool). Map each task to its bit issue by matching `task_id` embedded in the task issue title.
...
- TaskList task is `in_progress` and has unrecorded movement (commits, edited Target Files, decisions made) → plan a `Progress:` comment summarizing what advanced.
- TaskList task is `pending` and untouched → no comment.
- TaskList task is `completed`:
  - If matching bit issue is already closed → no action (hook already fired).
  - If matching bit issue is still open → call `TaskUpdate(task_id, completed)` to re-trigger the `TaskCompleted` hook. Only fall back to direct `bit issue close` if a previous re-trigger demonstrably failed (e.g., the issue is still open after the hook should have run). Always pair the close with a `Progress: done — <one line>` comment.
...
- Each non-obvious decision since last write → one `Decision: <choice> — <reason>` comment, placed as follows (rules cascade — apply the first matching one and stop):
  ...
  3. Else (decision is local to one task — e.g. a library/algorithm choice for that task's implementation) → comment on **that task's issue**. If multiple tasks are touched but only one is `in_progress`, treat the decision as local to the in-progress task (rule 3, not rule 2).
...
Render a compact summary of intended writes (what will be commented on which issue, what bodies will be rewritten). In **auto mode**, show the summary and proceed without confirmation. In normal mode, ask before applying.

Apply order:

1. Task body updates (scope changes via `bit issue update`).
2. Task comments (`Progress:`, `Decision:`, `Blocker:`, `Resolved:`).
3. `TaskUpdate(..., completed)` for each completed-but-open task — let the hook close.
...
Always `bit issue view <id>` immediately before any `bit issue update` — body is read-modify-write and you must not clobber concurrent edits from another session.

### Phase 5: Report

One-line per write, grouped by issue. Include parent snapshot timestamp so the user can correlate later. Then stop — do not continue making code changes implicitly. The user resumes regular work afterwards.
...
- Private repo names, company / org / client names, internal tool names — same rules as commit messages and PR descriptions in the global CLAUDE.md.
...
Running `write-session` twice in a row with no intervening change should produce a near-empty second pass: no body updates, no progress comments (nothing advanced), and **no snapshot** (per Phase 3's three-condition skip). Achieve this by:
...
| No open parent on branch                                              | Suggest `/start-work` (or `/restoring-session` if user expects an existing one). Stop.                                                                                     |
...
| Closed-but-still-open mismatch (TaskList completed, hook didn't fire) | First retry via `TaskUpdate(task_id, completed)`. If still open, fall back to direct `bit issue close` with a `Progress: done — closed by write-session fallback` comment. |

## Prohibited (relay/network)

Same as `start-work` and `restoring-session`. Never run: `bit issue claim` / `unclaim` / `claims` / `watch` / `import`, `bit pr import`, `bit relay serve` / `sync`, `bit clone relay+*`. `write-session` is local-only.

## Worked examples

For concrete write-session walk-throughs (routine checkpoint, plan-section rewrite, scope-change reconciliation, hook-fallback close), read `references/examples.md`.
